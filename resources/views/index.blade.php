							...
							<div class="pl-3 pt-3 pr-3 pb-2 pt-0">
								...
								<div class="pb-0 mt-0">
									<div class="d-flex">
										<div class="">
											<h4 class="tx-20 font-weight-bold mb-1 text-white">
                                                {{number_format(\App\Models\Invoice::where('status',1)->sum('total'),2)}}
                                            </h4>
											<p class="mb-0 tx-12 text-white op-7">{{\App\Models\Invoice::where('status',1)->count('total')}}</p>
										</div>
										<span class="float-right my-auto mr-auto">
											<i class="fas fa-arrow-circle-down text-white"></i>
											<span class="text-white op-7">
                                                {{round((\App\Models\Invoice::where('status',1)->count('total') / \App\Models\Invoice::count() * 100))}}%
                                            </span>
										</span>
									</div>
								</div>
							...
							<div class="pl-3 pt-3 pr-3 pb-2 pt-0">
								...
								<div class="pb-0 mt-0">
									<div class="d-flex">
										<div class="">
											<h4 class="tx-20 font-weight-bold mb-1 text-white">
                                                {{number_format(\App\Models\Invoice::where('status',0)->sum('total'),2)}}
                                            </h4>
											<p class="mb-0 tx-12 text-white op-7">{{\App\Models\Invoice::where('status',0)->count('total')}}</p>
										</div>
										<span class="float-right my-auto mr-auto">
											<i class="fas fa-arrow-circle-up text-white"></i>
											<span class="text-white op-7">
                                                {{round((\App\Models\Invoice::where('status',0)->count('total') / \App\Models\Invoice::count() * 100))}}%
							...
							<div class="pl-3 pt-3 pr-3 pb-2 pt-0">
								...
								<div class="pb-0 mt-0">
									<div class="d-flex">
										<div class="">
											<h4 class="tx-20 font-weight-bold mb-1 text-white">
                                                {{number_format(\App\Models\Invoice::where('status',2)->sum('total'),2)}}
                                            </h4>
											<p class="mb-0 tx-12 text-white op-7">{{\App\Models\Invoice::where('status',2)->count('total')}}</p>
										</div>
										<span class="float-right my-auto mr-auto">
											<i class="fas fa-arrow-circle-down text-white"></i>
											<span class="text-white op-7">
                                                {{round((\App\Models\Invoice::where('status',2)->count('total') / \App\Models\Invoice::count() * 100))}}%
...
<script>
    ...
    var notifications = notificationsWrapper.find('h6.notification-badge');
    var notifications2 = notificationsWrapper.find('h6.notification-count');

    // Enable pusher logging - don't include this in production
    Pusher.logToConsole = true;

    var pusher = new Pusher('d76fb312e56863b6b97b', {
      cluster: 'eu'
    });

    var channel = pusher.subscribe('invoice-channel');
    channel.bind('invoice-notification', function(data) {
      ...
      notifications.html(newNotificationHtml);
		...
		<h6 style="color: yellow" id="notifications_count" class="notification-count">
            عدد الاشعارات الغير مقروءة : ` + data.notification_count + `
        </h6>
        <div class="notification-label mb-1"></div>
        <a href="http://127.0.0.1:8000/invoices">
            <span style="color: yellow">تم اضافة فاتورة جديدة منذ دقيقة </span>
        </a>
       `;
